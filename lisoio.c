 ...
 */
ssize_t lisoio_sendn(int fd, SSL *ssl_context, char *ubuf, size_t n) {	
    size_t nleft = n;
    ssize_t nsend;
    char *buf = ubuf;
    if (ssl_context != NULL) {
 ...
 */
ssize_t lisoio_readn(int fd, SSL *ssl_context, char *buf, size_t n) {
	size_t res = 0;
	ssize_t nread;
	size_t nleft = n;
 ...
 */
ssize_t lisoio_recvlineb(int fd, SSL *ssl_context, void *usrbuf, size_t maxlen)
{
    int n, rc;
    char c, *bufp = usrbuf;
