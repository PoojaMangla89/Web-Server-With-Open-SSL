/** @file lisod.c          
 *  @brief This is a select()-based echo web server
 *  @author Pooja Mangla
 */
    ...
    for (i = 0; (i <= p->maxi) && (p->nready > 0); i++) {
        ...
        if (FD_ISSET(conn_sock, &p->ready_read)) {
            ...
            if (bufi->stage == STAGE_MUV) {
                ...
                readret = lisoio_recvlineb(conn_sock, client_context,
    ...
    for (i = 0; (i <= p->maxi) && (p->nready > 0); i++) {
        ...
        if (FD_ISSET(conn_sock, &p->ready_write)) {
            ...
            while (req) {
                ...
                if ((sendret = lisoio_sendn(conn_sock, client_context, 
                                         req->response, 
...
int read_requesthdrs(Buff *b, Requests *req) {
    ...
    while (1) {
        buf += len;
        lisoio_recvlineb(b->fd, b->client_context, buf, b->size - b->cur_size);
        len = strlen(buf);
